...
Non-primitive[modifiable or mutable] data types in JavaScript includes:
...
- *Arrays can contain the same or different data types. Array values are referenced by their index. In JavaScript array index starts at zero. I.e., the first element of an array is found at index zero, the second element at index one, and the third element at index two, etc.*
...
Using a string template (Template String Literals/string interpolation method), we can add expressions, which could be a value, or some operations.

(comparison, arithmetic operations, ternary operation[a ternary operator is an operator that takes three arguments.])
...
2. *Accessing characters in a string*: We can access each character in a string using its index. In programming js, counting starts from 0. 
...
7. *split()*: The split method splits a string at a specified place and turns it into an array.
...
9. *includes()*: It takes a substring argument and it checks if substring argument exists in the string (case sensitive). 
...
13.  *indexOf()*: Takes a substring and if the substring exists in a string it returns the first position of the substring if it does not exist it returns -1
